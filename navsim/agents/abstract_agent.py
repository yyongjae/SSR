...
class AbstractAgent(torch.nn.Module, ABC):
    ...
    def compute_trajectory_gpu(self, agent_input: AgentInput) -> Trajectory:
        ...
        # add batch dimension
        # Follow the model placement chosen by the caller instead of assuming
        # the default CUDA device.  This matters for CUDA_VISIBLE_DEVICES,
        # non-zero local ranks, and the CPU fallback in the GPU scorer.
